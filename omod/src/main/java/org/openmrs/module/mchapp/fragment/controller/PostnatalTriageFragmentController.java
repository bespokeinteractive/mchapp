...
public class PostnatalTriageFragmentController {
	...
	public void controller(FragmentModel model, FragmentConfiguration config, 
		...
		if (StringUtils.isNotEmpty(encounterId)) {
			Encounter current = Context.getEncounterService().getEncounter(
					Integer.parseInt(encounterId));
			Set<Obs> obs = Context.getEncounterService()
					.getEncounterByUuid(current.getUuid()).getAllObs();
			for (Obs s : obs) {
